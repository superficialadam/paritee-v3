...
// Default parameters for background pages
const params = {
  // Camera controls
  cameraOffsetX: 0.0,
  cameraOffsetY: 0.0,
  ...
  // Dot matrix controls
  gridResolution: 80, // Lower for background pages
  ...
  // Main noise controls - Generic background defaults
  mainNoiseEnabled: true, // Toggle main noise on/off
  noiseScale: 80.0, // Larger scale for background
  noiseSpeed: 4.0, // Slower animation
  noiseOctaves: 4, // Number of octaves for fractal noise
  noiseLacunarity: 2.0, // Frequency multiplier per octave
  noiseGain: 0.673, // Amplitude multiplier per octave
  noiseThreshold: 0.4, // Black level threshold (cuts off values below)
  noiseIslandSize: 0.8937, // Controls bright island size (remap range)
  noiseExposure: 0.742, // Exposure adjustment (stops)
  noiseGamma: 0.5, // Gamma correction (1 = linear)
  noiseMultiplier: 0.4, // Final output multiplier (overall fader)
  ...
  // Influence zone 1 controls - Disabled for background
  influence1Enabled: false,
  ...
  // Influence zone 2 controls - Disabled for background
  influence2Enabled: false,
...
function fbm3D(x, y, z, octaves, lacunarity, gain, seed = 0) {
  ...
  for (let i = 0; i < octaves; i++) {
    value +=
      amplitude * gradientNoise3D(x * frequency, y * frequency, z * frequency);
...
function calculateInfluence(
  ...
  if (zoneParams.edgeEnabled && distance > zoneParams.edgeStart) {
    ...
    const edgeWeight =
      (distance - zoneParams.edgeStart) / (1.0 - zoneParams.edgeStart);

    // ADD edge noise (not multiply) - edge noise adds or subtracts from influence
    const noiseEffect =
      (edgeNoise - 0.5) * 2 * zoneParams.edgeInfluence * edgeWeight;
...
function createDotMatrix() {
  ...
  dotMaterial = new THREE.ShaderMaterial({
    ...
    uniforms: {
      ...
      uInfluence1Pos: {
        value: new THREE.Vector2(params.influence1X, params.influence1Y),
      },
      uInfluence1Radius: {
        value: new THREE.Vector2(
          params.influence1RadiusX,
          params.influence1RadiusY,
        ),
      },
      ...
      uInfluence2Pos: {
        value: new THREE.Vector2(params.influence2X, params.influence2Y),
      },
      uInfluence2Radius: {
        value: new THREE.Vector2(
          params.influence2RadiusX,
          params.influence2RadiusY,
        ),
      },
...
function updatePoint(index, properties = {}) {
  ...
  if (properties.color !== undefined) {
    ...
    dotAttributes.colors[colorIndex] =
      properties.color.r || properties.color[0] || 0;
    dotAttributes.colors[colorIndex + 1] =
      properties.color.g || properties.color[1] || 0;
    dotAttributes.colors[colorIndex + 2] =
      properties.color.b || properties.color[2] || 0;
...
function syncParamsToUniforms() {
  ...
  dotMaterial.uniforms.uInfluence1Pos.value.set(
    params.influence1X,
    params.influence1Y,
  );
  dotMaterial.uniforms.uInfluence1Radius.value.set(
    params.influence1RadiusX,
    params.influence1RadiusY,
  );
  ...
  dotMaterial.uniforms.uInfluence1EdgeEnabled.value =
    params.influence1EdgeEnabled;
  dotMaterial.uniforms.uInfluence1EdgeStart.value = params.influence1EdgeStart;
  dotMaterial.uniforms.uInfluence1EdgeInfluence.value =
    params.influence1EdgeInfluence;
  ...
  dotMaterial.uniforms.uInfluence1EdgeOctaves.value =
    params.influence1EdgeOctaves;
  dotMaterial.uniforms.uInfluence1EdgeLacunarity.value =
    params.influence1EdgeLacunarity;
  dotMaterial.uniforms.uInfluence1EdgeGain.value = params.influence1EdgeGain;
  dotMaterial.uniforms.uInfluence1EdgeThreshold.value =
    params.influence1EdgeThreshold;
  dotMaterial.uniforms.uInfluence1EdgeIslandSize.value =
    params.influence1EdgeIslandSize;
  dotMaterial.uniforms.uInfluence1EdgeExposure.value =
    params.influence1EdgeExposure;
  ...
  dotMaterial.uniforms.uInfluence2Pos.value.set(
    params.influence2X,
    params.influence2Y,
  );
  dotMaterial.uniforms.uInfluence2Radius.value.set(
    params.influence2RadiusX,
    params.influence2RadiusY,
  );
  ...
  dotMaterial.uniforms.uInfluence2EdgeEnabled.value =
    params.influence2EdgeEnabled;
  dotMaterial.uniforms.uInfluence2EdgeStart.value = params.influence2EdgeStart;
  dotMaterial.uniforms.uInfluence2EdgeInfluence.value =
    params.influence2EdgeInfluence;
  ...
  dotMaterial.uniforms.uInfluence2EdgeOctaves.value =
    params.influence2EdgeOctaves;
  dotMaterial.uniforms.uInfluence2EdgeLacunarity.value =
    params.influence2EdgeLacunarity;
  dotMaterial.uniforms.uInfluence2EdgeGain.value = params.influence2EdgeGain;
  dotMaterial.uniforms.uInfluence2EdgeThreshold.value =
    params.influence2EdgeThreshold;
  dotMaterial.uniforms.uInfluence2EdgeIslandSize.value =
    params.influence2EdgeIslandSize;
  dotMaterial.uniforms.uInfluence2EdgeExposure.value =
    params.influence2EdgeExposure;
...
function setupGUI() {
  ...
  cameraFolder
    .add(params, "cameraOffsetX")
    .name("X Position")
    .onChange((v) => (camera.position.x = v));
  cameraFolder
    .add(params, "cameraOffsetY")
    .name("Y Position")
    .onChange((v) => (camera.position.y = v));
  cameraFolder
    .add(params, "cameraOffsetZ")
    .name("Z Position")
    .min(0.1)
    .onChange((v) => {
      camera.position.z = v;
  ...
  matrixFolder
    .add(params, "gridResolution")
    .name("Grid Resolution")
    .min(1)
    .step(1)
    .onChange(() => createDotMatrix());
  matrixFolder
    .add(params, "sizeBlack")
    .name("Size (Black)")
    .min(0)
    .onChange((v) => {
  ...
  matrixFolder
    .add(params, "pointMargin")
    .name("Point Margin")
    .min(0)
    .onChange(() => createDotMatrix());
  matrixFolder
    .add(params, "pointOpacity")
    .name("Global Opacity")
    .min(0)
    .max(1)
    .onChange((v) => {
  ...
  influence1Folder
    .add(params, "influence1Intensity")
    .name("Intensity")
    .onChange((v) => {
  ...
  edge1Folder
    .add(params, "influence1EdgeInfluence")
    .name("Influence")
    .min(0)
    .onChange((v) => {
  ...
  edge1Folder
    .add(params, "influence1EdgeScale")
    .name("Scale")
    .min(0)
    .onChange((v) => {
  ...
  edge1Folder
    .add(params, "influence1EdgeSpeed")
    .name("Speed")
    .min(0)
    .onChange((v) => {
  ...
  edge1Folder
    .add(params, "influence1EdgeOctaves")
    .name("Octaves")
    .min(1)
    .step(1)
    .onChange((v) => {
  ...
  edge1Folder
    .add(params, "influence1EdgeLacunarity")
    .name("Lacunarity")
    .min(0)
    .onChange((v) => {
  ...
  influence2Folder
    .add(params, "influence2Intensity")
    .name("Intensity")
    .onChange((v) => {
  ...
  edge2Folder
    .add(params, "influence2EdgeInfluence")
    .name("Influence")
    .min(0)
    .onChange((v) => {
  ...
  edge2Folder
    .add(params, "influence2EdgeScale")
    .name("Scale")
    .min(0)
    .onChange((v) => {
  ...
  edge2Folder
    .add(params, "influence2EdgeSpeed")
    .name("Speed")
    .min(0)
    .onChange((v) => {
  ...
  edge2Folder
    .add(params, "influence2EdgeOctaves")
    .name("Octaves")
    .min(1)
    .step(1)
    .onChange((v) => {
  ...
  edge2Folder
    .add(params, "influence2EdgeLacunarity")
    .name("Lacunarity")
    .min(0)
    .onChange((v) => {
  ...
  const debugParams = {
    ...
    testUniforms: () => {
      if (dotMaterial) {
        ...
        console.log(
          "uMainNoiseEnabled:",
          dotMaterial.uniforms.uMainNoiseEnabled.value,
        );
...
window.dotMatrixEnabled = true;

renderer.setAnimationLoop(() => {
  if (!window.dotMatrixEnabled) return; // Skip rendering if disabled
