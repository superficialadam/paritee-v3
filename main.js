...
window.addEventListener("load", function () {
  ...
  utils.set(heroPrismHeadings[0], {
    opacity: 0,
    x: window.innerWidth,
    filter: "blur(5px)",
  }); // cyan
  utils.set(heroPrismHeadings[1], {
    opacity: 0,
    x: window.innerWidth + 30,
    filter: "blur(5px)",
  }); // purple
  utils.set(heroPrismHeadings[2], {
    opacity: 0,
  ...
  const heroLogoRevealTl = createTimeline({
    ...
    .add(
      dotMatrixParams,
      {
        ...
        ease: "outExpo",
        onUpdate: () =>
          window.syncParamsToUniforms && window.syncParamsToUniforms(),
      },
      500,
    )
    .add(dotMatrixParams, {
    ...
    .add(
      logo3,
      {
        ...
        onComplete: () => {
          ...
          if (menuWrapper) {
            menuWrapper.classList.add("faded-in");
          } else {
  ...
  const headingContainers = document.querySelectorAll(
    ".section:not(#hero) .heading-container",
  );
  ...
  headingContainers.forEach(function (container) {
    ...
    utils.set(prismHeadings[0], {
      opacity: 0,
      x: window.innerWidth,
      filter: "blur(5px)",
    }); // cyan
    utils.set(prismHeadings[1], {
      opacity: 0,
      x: window.innerWidth + 30,
      filter: "blur(5px)",
    }); // purple - slightly offset
    utils.set(prismHeadings[2], {
      opacity: 0,
      x: window.innerWidth + 60,
      filter: "blur(5px)",
    }); // green - more offset
  ...
  headingContainers.forEach(function (container) {
    ...
    if (homeContent) {
      headingTimeline.add(
        ...
        prismTime + 500,
      ); // Start slightly after the main heading appears
    }
  });
  ...
  const updateCirclesOnScroll = () => {
    ...
    if (window.dotMatrixParams && fromConfig.dotMatrix && toConfig.dotMatrix) {
      ...
      dotMatrixParams.influence1X =
        fromDot.influence1X +
        (toDot.influence1X - fromDot.influence1X) * localProgress;
      dotMatrixParams.influence1Y =
        fromDot.influence1Y +
        (toDot.influence1Y - fromDot.influence1Y) * localProgress;
      dotMatrixParams.influence1RadiusX =
        fromDot.influence1RadiusX +
        (toDot.influence1RadiusX - fromDot.influence1RadiusX) * localProgress;
      dotMatrixParams.influence1RadiusY =
        fromDot.influence1RadiusY +
        (toDot.influence1RadiusY - fromDot.influence1RadiusY) * localProgress;
      dotMatrixParams.influence1Falloff =
        fromDot.influence1Falloff +
        (toDot.influence1Falloff - fromDot.influence1Falloff) * localProgress;
      dotMatrixParams.influence1Intensity =
        fromDot.influence1Intensity +
        (toDot.influence1Intensity - fromDot.influence1Intensity) *
          localProgress;
      dotMatrixParams.influence1EdgeStart =
        fromDot.influence1EdgeStart +
        (toDot.influence1EdgeStart - fromDot.influence1EdgeStart) *
          localProgress;
      dotMatrixParams.influence1EdgeInfluence =
        fromDot.influence1EdgeInfluence +
        (toDot.influence1EdgeInfluence - fromDot.influence1EdgeInfluence) *
          localProgress;
      dotMatrixParams.influence1EdgeScale =
        fromDot.influence1EdgeScale +
        (toDot.influence1EdgeScale - fromDot.influence1EdgeScale) *
          localProgress;
      dotMatrixParams.influence1EdgeSpeed =
        fromDot.influence1EdgeSpeed +
        (toDot.influence1EdgeSpeed - fromDot.influence1EdgeSpeed) *
          localProgress;
      dotMatrixParams.influence1EdgeGain =
        fromDot.influence1EdgeGain +
        (toDot.influence1EdgeGain - fromDot.influence1EdgeGain) * localProgress;
      dotMatrixParams.influence1EdgeThreshold =
        fromDot.influence1EdgeThreshold +
        (toDot.influence1EdgeThreshold - fromDot.influence1EdgeThreshold) *
          localProgress;
      dotMatrixParams.influence1EdgeIslandSize =
        fromDot.influence1EdgeIslandSize +
        (toDot.influence1EdgeIslandSize - fromDot.influence1EdgeIslandSize) *
          localProgress;
      dotMatrixParams.influence1EdgeExposure =
        fromDot.influence1EdgeExposure +
        (toDot.influence1EdgeExposure - fromDot.influence1EdgeExposure) *
          localProgress;
      dotMatrixParams.influence1EdgeGamma =
        fromDot.influence1EdgeGamma +
        (toDot.influence1EdgeGamma - fromDot.influence1EdgeGamma) *
          localProgress;
  ...
  if (globeSections.length > 0) {
    ...
    const updateGlobeScroll = () => {
      if (
        window.globeRotation === undefined ||
        window.globeOpacity === undefined
      )
        return;
      ...
      const newRotation =
        startRotation + (endRotation - startRotation) * clampedProgress;
